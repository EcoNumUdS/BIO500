...
dbSendQuery(ma_bd, create_mtcars)
...
Pour envoyer des instructions, c'est la commande `dbGetQuery()` qui est utilisée. Sont premier argument est l'objet contenant la connexion et le second les instructions SQL.

```R
resr <- dbGetQuery(ma_bd, 'SELECT * FROM mtcars LIMIT 5')
res
...
Une requête peut retourner un champ agrégé, par exemple le résultat d'un produit et filtrer les lignes retournées en fonction du résultat de l'agrégation avec `HAVING` :
