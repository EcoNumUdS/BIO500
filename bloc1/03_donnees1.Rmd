...
- Chaque attribut de la table (`author`,`status` etc) dispose d'un type de données (`varchar(40)`,`DATE`,  etc) [Type de données SQLite](https://www.sqlite.org/datatype3.html)
...
.pull-right[
- On déclare `author1`, `author2` et `articleID` comme étant la clé primaire de la table `collaborations`.
- On référence chacun de ces trois attributs comme étant la clé étrangère
]
