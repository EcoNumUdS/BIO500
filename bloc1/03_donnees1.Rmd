...
Biodiversité Québec développe une infrastructure informatique qui servira à documenter en temps réel
l'état de la biodiversité et ses changements

### Rôles
- Numériser et intégrer des données à la base de données
- Participation aux activités de développement éco-informatiques
- Participation à diverses activité en fonction des aptitudes et des intérêts
- Participation aux rencontres d'équipe, avec le ministère et avec les partenaires

### Exigences

Les outils utilisés (SQL, R, Git) seront couverts dans le cours
...
Nous avons défini les champs à documenter.

## Aujourd'hui

Collecter des données : format et types de données
...
## Les données devront être déposées sur Moodle d'ici vendredi.

Les entrées dans la table *collaborations* doivent être faites **dans les deux sens** (auteur1, auteur2) et (auteur2, auteur1).
...
Évaluation formative à remettre pour le 11 mars sur Moodle (ATELIER Assemblage, nettoyage et validation des données)

Vous avez à soumettre vos scripts qui servent à assembler, nettoyer et valider les données. Vos scripts doivent produire une ou plusieurs tables de données (dataframe) prêtes à être injectées. Chacun de ces dataframes peut représenter une table de la base de données à venir.
...
- Spécifiez pour quel jeu de donnée les scripts sont utilisés
- Commentez vos scripts pour que votre processus soit évident aux autres
- Divisez les tâches en fonctions distinctes
- Un script par fonction
- Un script principal qui décrit le processus et fait appel aux fonctions qui exécutent les tâches
- Vos scripts doivent produire un ou des dataframes prêts à être injectés dans la base de données. Chacun des dataframes doit représenter une table de la base de données
...
Bloc 1 > Assemblage, nettoyage et validation des données > Formulaire d'évaluation
...
.center[
<img src="assets/img/grille_eval1.png" width="90%"></img>
]

<!-- Ajouter grille d'évaluation sur Moodle -->
