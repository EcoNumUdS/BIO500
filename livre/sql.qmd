...
Le `SQL` est un langage informatique créé spécifiquement pour manipuler des base de données relationnelles. C'est le langage utilisé pour transmettre nos instrusctions au SGBD. Il permet de :
...
> Le langage de programmation SQL et son utilisation sont détaillées dans la section [Utilisation avancée du SQL](http://localhost:4345/bases_de_donnees.html#utilisation-avanc%C3%A9e-du-sql).
...
dbSendQuery(con, create_iris)
```


### 3. Injecter des données

Une table de données peut facilement être copiée dans une base de données SQLite avec `dbWriteTable()`. Pour ce faire, les données doivent avoir le même format que la table dans laquelle elles sont insérées :

```R
ma_db <- dbConnect(RSQLite::SQLite(), dbname="./my_database.db")

# Injection des enregistrements dans la B
dbWriteTable(con, append = TRUE, name = "mtcars", value = mtcars, row.names = FALSE)
dbWriteTable(con, append = TRUE, name = "iris", value = iris, row.names = FALSE)
...
Pour envoyer des instructions, c'est la commande `dbGetQuery()` qui est utilisée. Sont premier argument est l'object contenant la connexion et le second les instructions SQL.

```R
resr <- dbGetQuery(mydb, 'SELECT * FROM mtcars LIMIT 5')
res
...
Les requêtes serviront à créer les tables de la base de données, à spécifier les clés, à filtrer les données et à extraire les données de la dase de données. Pour ce faire, une certaine maitrise du langage SQL est nécessaire.
...
`AS` permet de renommer un champ dans la table retournée. La table table1 d'où les données proviennent n'est pas effecté par la commande. Cet astuce facilite l'organisation lorsque plusieurs champs ont le même nom ou que plusieurs champs sont aggrégés en une seule colonne :
...
Les opérateur de comparaison standards tels que `>=`, `<=`, `=` sont accessibles pour les valeurs numériques.

#### Champs de texte

Les valeurs de texte sont filtrés avec `LIKE` et `NOT LIKE` :
...
`%` tient pour n'importequel caractère (Lyne Beauchamp, Julien Beauchamp-Lavallée) alors que `_` est utilisé pour un seul caractère (`WHERE nom LIKE 'Beauchamp_'` : Beauchamp, Beauchamps, etc.).
...
On peut aussi effecturer des filtres multi-critères avec `AND` et `OR` :
...
Une requête peut retourner un champ agrégé, par exemple le résultat d'un produit et filtrer les lignes retournées en fonction du résultat de l'aggrégation avec `HAVING` :

```SQL
SELECT id, nom, no_telephonne, date_de_naissance * 2 AS annee2
FROM client
HAVING annee2 >= 4000;"
```
...
`DISTINCT` retourne les valeurs distinctes d'un champs :
...
`ORDER BY` permet d'ordonner les rangées selon 
...
`GROUP BY` définit les champs sur lequel se fera l'agrégation des données. L'aggrégration se fait dans la sélection des champs :
...
Le `LEFT JOIN` est la jointure la plus populaire puisqu'elle conserve les lignes de la table principale (de gauche) et ne joint que les colonnes de la seconde table (de droite) aux lignes correspondantes. Notez que lorsqu'il n'y a pas de correspondance, une valeure `NULL` est retournée :
...
Le `INNER JOIN` ne renvoie que rangées des tables ayant un champ contenant une valeur commune. La requête suivante ne retourne que 4 lignes puisque la table magasin ne possède pas de id avec une valeur de 8 :
