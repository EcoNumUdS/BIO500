...
Précédemment, nous avons vu les aspects important de la conceptualisation d'une base de données avec les entités, les relations, les attributs et les types. Voyons maintenant comment passer au modèle informatique.
...
Pour faciliter l'interaction avec la base de données et simplifier les manipulations, nous utiliserons `RStudio` et la librairie `RSQLite` qui contient le client `SQLite3` pour interagir avec la base de données. 

D'abord, il faut se connecter à la base de données à l'aide de la commande `dbConnect()` et sauver cette connexion dans un objet. Nous réutiliserons cet objet à toutes les fois que nous enverrons une commande à la base de données pour spécifier la connection client-serveur.
...
Ici, `con` est un objet contenant la connexion avec le serveur/fichier de base de données. Pour envoyer des instructions, c'est la commande `dbSendQuery()` qui est utilisée. Sont premier argument est l'object contenant la connexion et le second les instructions `SQL`.

```R
dbSendQuery(con,"Instructions SQL à envoyer;")
```

Il est à noter que le code `SQL` est enchassé entre `""` pour former une chaine de charactère. **Il s'agit d'une cause d'erreur fréquente !!!**
