...
```{r, eval=FALSE}
# Exemple : dupliquer les données pour augmenter le volume
data_big1 <- data[rep(seq_len(nrow(data)), times = 1000), ]
data_big2 <- data[rep(seq_len(nrow(data)), times = 2000), ]
data_big3 <- data[rep(seq_len(nrow(data)), times = 4000), ]
```

Puis, relancez le script de calcul du nombre de plantes par site avec chacun des nouveaux jeux de données :
...
system.time({                           # system.time() pour mesurer le temps d'exécution
    result_1 <- data.frame()
    for (inst in unique(data_big1$site)) {
        n <- sum(data_big1$site == inst)
        result_1 <- rbind(
            result_1,
            data.frame(site = inst, n_plantes = n)
        )
...
# Trier les résultats en ordre décroissant
result <- result_1[order(result_1$n_plantes, decreasing = TRUE), ]

## data_big2
system.time({                           # system.time() pour mesurer le temps d'exécution
    result_2 <- data.frame()
    for (inst in unique(data_big2$site)) {
        n <- sum(data_big2$site == inst)
        result_2 <- rbind(
            result_2,
            data.frame(site = inst, n_plantes = n)
        )
...
# Trier les résultats en ordre décroissant
result <- result_2[order(result_2$n_plantes, decreasing = TRUE), ]

## data_big3
system.time({                           # system.time() pour mesurer le temps d'exécution
    result_3 <- data.frame()
    for (inst in unique(data_big3$site)) {
        n <- sum(data_big3$site == inst)
        result_3 <- rbind(
            result_3,
            data.frame(site = inst, n_plantes = n)
        )
...
# Trier les résultats en ordre décroissant
result <- result_3[order(result_3$n_plantes, decreasing = TRUE), ]
```
...
✍️ Notez ce que vous observez :
- Temps d'exécution data_big1 :
- Temps d'exécution data_big2 :
- Temps d'exécution data_big3 :
...
# Calcul : nombre de plantes par site
system.time({                           # system.time() pour mesurer le temps d'exécution
    result_modif <- data.frame()
    for (inst in unique(data_modif$site)) {
        n <- sum(data_modif$site == inst)
        result_modif <- rbind(
            result_modif,
            data.frame(site = inst, n_plantes = n)
        )
...
# Trier les résultats en ordre décroissant
result_modif <- result_modif[order(result_modif$n_plantes, decreasing = TRUE), ]

# Afficher les résultats
summary(result_modif)
print(result_modif)
```
...
Essayez d'adapter le code de calcul pour le nouveau jeu de données suivant légèrement différent :
...
Code de calcul à adapter. Le code suivant est celui utilisé pour le jeu de données initial, vous devez l'adapter pour qu'il fonctionne avec le nouveau jeu de données :
...
Assemblez le calcul dans une fonction distincte et complétez le code suivant. Notez les paramètres utilisés. Ici, il n'y a pas de valeurs par défaut, mais vous pouvez en ajouter si vous le souhaitez.
