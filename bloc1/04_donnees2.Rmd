...
2. Insérer les données [auteurs.csv](./assets/data/db_reseau/auteurs.csv),[collaborations.csv](./assets/data/db_reseau/collaborations.csv) et [articles.csv](./assets/data/bd_beacon/bd_films.csv) dans les deux tables à l'aide de la commande R `dbWriteTable()`
...
.pull-left[
```{r, eval = FALSE}
sql_requete <- "SELECT * FROM articles LIMIT 10;"
...
- Cette requête retournera toutes les colonnes de la table `articles`
...
.pull-left[
```{r}
sql_requete <- "SELECT DISTINCT auteur
FROM auteurs LIMIT 10;"
...
```{r, eval = FALSE}
sql_requete <- "
SELECT titre, annee, citations, articleID
...
```{r, echo = FALSE}
sql_requete <- "
SELECT titre, annee, articleID
  FROM articles ORDER BY annee DESC
;"
derniers_articles <- dbGetQuery(con, sql_requete)
head(derniers_articles)
```
...
```{r}
sql_requete <- "
SELECT count(articleID) AS nb_articles, annee
  FROM articles
  GROUP BY annee;"
...
```{r}
sql_requete <- "
SELECT titre, journal, articles.articlesID, auteurs.articleID
  FROM articles
  INNER JOIN auteurs ON articles.articlesID = auteurs.articleID
  ;"
...
SELECT titre, journal, articles.articlesID, auteurs.articleID
  FROM articles
  INNER JOIN auteurs USING (articleID)
  ;"
...
## Quel auteur a publié le plus d'articles ?
...
```sql
UPDATE auteurs SET institution = 'UdeS' WHERE genre = 'Universite de Sherbrooke';
```
