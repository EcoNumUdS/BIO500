...
Le package `knitr` permet de faciliter cette procédure en exportant le `data.frame` directement dans son document de travail RMarkdown.
...
Je souhaite maintenant exporter ce `data.frame` en markdown:

```{r}
library(knitr)
iris_tex <- kable(iris,format="pipe")
```
