...
Ici, il est bien important de garder en tête que l'on défini une *séquence d'instructions*. Une étape (appelée target) dépend de ce qui est produit à l'étape précédante pour exécuter une opération et produire un objet qui sera utilisé par la prochaine étape. 
...
Le fichier _targets.R se subdivise en 3 sections. D'abord, les dépendances à la librairie targets et aux autres libraries. Ensuite, l'exécution des scripts R qui contiennent les fonctions du pipeline. Finalement, le pipeline composé d'une liste contenant une série de targets.
