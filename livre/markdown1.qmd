...
Le RMarkdown est également une librairie R. Vous aurez dabord à l'installer.
...
> Notez que le RMarkdown est un lagage compilé, contrairement à R qui est interprété. C'est-à-dire, que le fichier RMarkdown doit être "knit" dans sont entièreté pour produire le document final (html, pdf ou Word).
...
1. avoir la librarie installée sur sont ordinateur
2. creer nouveau document RMarkdown à partir d'un gabarit (New File > RMarkdown)
...
Un fichier RMarkdown est composée de 3 structures : une portion YAML, une portion Markdown et des blocs de code. Chacune de ces structures occupe une fonction distincte.

### YAML

Le YAML est toujours situé au haut du document et défini les métadonnées et les options du document. C'est dans cette section séparée par `---` à son début et à sa fin que les informations sur le document et les paramètres servant à la mise en page se trouvent. Les paramètres suivent la syntaxe clé: valeur.
...
Un bloc de code peut être ajouté au document en sélectionnant l'icône vert du panneau RStudio. Le code R doit être à l'intérieur d'un bloc de code (*code chunk*). Par exemple:
...
Une bonne pratique est de nommer les blocs de code. Cela s'avère utile pour débogage. Deux blocs de code avec le même nom (label) retournera un message d'erreur `Duplicate chunk label 'chunk name', which has been used for the chunk`.
